...
def parse_pdf_from_blob(file, form_recognizer=False, model="prebuilt-document", verbose=False):
    offset = 0
    page_map = []
...
def get_search_results(query: str, indexes: list, 
    ...
    for index in indexes:
        ...
        if vector_search:
            ...
            search_payload["select"]= "id, content, filepath, title, url"
        else:
            search_payload["select"]= "id, content, filepath, title, url"
...
class DocSearchTool(BaseTool):
    ...
    def qa_chain_agent(self, QUESTION: str, vector_indexes: List[str] = []):
        k = 10
        similarity_k = 5
        embedder = OpenAIEmbeddings(deployment=self.embedding_model, chunk_size=1)
        ...
        for key,value in ordered_results.items():
            location = value["url"] if value["url"] is not None else ""
            top_docs.append(Document(page_content=value["content"], metadata={"source": location}))

        llm = AzureChatOpenAI(deployment_name="gpt-35-turbo-16k", temperature=0.2, max_tokens=800)
        # llm = AzureChatOpenAI(deployment_name="gpt-4-32k", temperature=0.2, max_tokens=800)
        ...
        try:
            ...
            for i in range(2):
                try:
                    ...
                    response = self.qa_chain_agent(parsed_input, vector_indexes=self.vector_only_indexes)
                    break
                except Exception as e:
                    response = str(e)
                    continue
